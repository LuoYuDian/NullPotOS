...
void VGA_put_char_color(char chr,VGA color)
{
    if (chr == '\n') 
    {
        VGA_COLUMN = 0;
    ...
    {
        *(buffer + VGA_COLUMN + VGA_ROW * VGA_WIDTH) = (color | chr);
        VGA_COLUMN++;
        if (VGA_COLUMN >= VGA_WIDTH) 
        {
            VGA_COLUMN = 0;
...
void VGA_put_str_color(char* str,VGA color)
{
    while (*str != '\0') 
    {
...
void VGA_clear_color(VGA color)
{
    for (uint32_t x = 0; x < VGA_WIDTH; x++) 
    {
        for (uint32_t y = 0; y < VGA_HEIGHT; y++) 
        {
            *(buffer + x + y * VGA_WIDTH) = color;
        }
    }
    VGA_ROW = 0;
    VGA_COLUMN = 0;
